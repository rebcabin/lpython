...
def match_integer(candidate : str) -> bool:
    ...
    if candidate[0] == '0':
        ...
        if (candidate[1] == 'o') or (candidate[1] == 'O'):
            pass
    ...
    c : str
    for c in candidate:
        if not check_re_range(digit_range, c):
            return False
    return True
...
def regex_test() -> None:
    ...
    assert not match_integer('_42')  # Consider allowing
    # assert match_integer('42_0')
    assert not match_integer('0xBADBEEF')  # TODO: allow
